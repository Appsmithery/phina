        ...
        <View style={styles.wineHeroRow}>
          {hideDetails ? (
            <View style={[styles.wineThumbPlaceholder, { backgroundColor: theme.border }]}>
              <Ionicons name="eye-off-outline" size={28} color={theme.textMuted} />
            </View>
          ) : photoUrl ? (
          ...
          <View style={styles.wineInfo}>
            ...
            <Text style={[styles.wineMeta, { color: theme.textSecondary }]} numberOfLines={1}>
              {hideDetails ? "Details hidden until event ends" : [wine.varietal, wine.vintage?.toString(), wine.region].filter(Boolean).join(" · ")}
            </Text>
          </View>
          {eventId && userId && (
            <TouchableOpacity onPress={toggleFavorite} style={styles.favoriteBtn} hitSlop={12}>
              <Ionicons name={isFavorite ? "star" : "star-outline"} size={26} color={isFavorite ? theme.primary : theme.textMuted} />
            </TouchableOpacity>
          )}
        </View>
      </View>
