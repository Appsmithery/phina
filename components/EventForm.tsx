...
export function normalizePartifulUrl(value: string): string | null {
...
export function EventForm({
  ...
  const [partifulUrl, setPartifulUrl] = useState(initialValues.partifulUrl);
  const [selectedDate, setSelectedDate] = useState(new Date(`${initialValues.date}T00:00:00`));
  const [showPicker, setShowPicker] = useState(false);
  const [tastingMode, setTastingMode] = useState<"single_blind" | "double_blind">(initialValues.tastingMode);

  useEffect(() => {
    setTitle(initialValues.title);
    setThemeText(initialValues.theme);
    setDescription(initialValues.description);
    setPartifulUrl(initialValues.partifulUrl);
    setSelectedDate(new Date(`${initialValues.date}T00:00:00`));
    setTastingMode(initialValues.tastingMode);
  }, [
    initialValues.date,
    initialValues.description,
    initialValues.partifulUrl,
  ...
  const handleSubmit = () => {
    void onSubmit({
      ...
      partifulUrl: normalizePartifulUrl(partifulUrl),
      date,
      tastingMode,
    });
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.keyboardView}>
        ...
        >
          ...
          <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            ...
            <View style={[styles.modeRow, { borderColor: theme.border, opacity: tastingModeLocked ? 0.6 : 1 }]}>
            ...
            <Text style={[styles.modeHint, { color: theme.textSecondary }]}>
              {tastingModeLocked
                ? tastingModeLockedReason ?? "Tasting mode is locked once wines or ratings exist."
