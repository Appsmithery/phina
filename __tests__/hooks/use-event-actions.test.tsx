...
const mockInvoke = jest.fn<
  ...
  [string, { body: { event_id: string; wine_id: string }; headers?: Record<string, string> }]
>(
...
describe("useEndEvent", () => {
  ...
  it("tracks rating round start and push success", async () => {
    const { result } = renderHook(() => useStartRatingRound("event-1", "wine-1"), { wrapper: createWrapper() });
    ...
    await waitFor(() => expect(mockInvoke).toHaveBeenCalledWith("send-rating-round-push", {
      body: { event_id: "event-1", wine_id: "wine-1" },
  ...
  it("tracks missing-session push failure without invoking the function", async () => {
    ...
    const { result } = renderHook(() => useStartRatingRound("event-2", "wine-2"), { wrapper: createWrapper() });
  ...
  it("tracks rating round push failure", async () => {
    ...
    const { result } = renderHook(() => useStartRatingRound("event-3", "wine-3"), { wrapper: createWrapper() });
